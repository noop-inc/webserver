...
const InternalStream = require('./streams/InternalStream')
...
class Proxy extends EventEmitter {
  ...
  send (stream) {
    ...
    stream.proxy = this
    if (!stream.webserver.config.trust || !stream.request.headers['x-forwarded-proto']) {
      stream.request.headers['x-forwarded-proto'] = stream.session.secure ? 'https' : 'http'
    }
    ...
    stream.proxyTimeout = setTimeout(() => {
      ...
      if (index !== -1) {
        ...
        stream.webserver.emit('log', 'stream.proxy.timeout', {
          traceId: stream.traceId,
          destination: this.name,
  ...
  createStream (headers) {
    const stream = new InternalStream(headers)
    this.send(stream)
    return stream
  }
