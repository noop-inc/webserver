...
class Http2Agent extends Agent {
  connect (done) {
    ...
    this.http2Session.on('timeout', err => this.handleClose(err))
    if (this.proxy.config.keepAlive) this.pingInterval = setInterval(() => this.ping(), 10000)
    this.ping()
  }
  ...
  ping () {
    ...
    this.http2Session.ping(null, (err, duration) => {
      if (err) return this.handleClose()
      this.proxy.latency = duration
    })
  }
}
