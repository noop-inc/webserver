...
class Stream extends EventEmitter {
  ...
  error (err) {
    ...
    const body = { error, message, ...err.data }
  ...
  tunnel (params) {
    if (params.port && !isNaN(params.port)) params.port = parseInt(params.port)
    this.webserver.emit('log', 'stream.tunnel', {
      traceId: this.traceId,
      streamId: this.id,
      params
    })
    this.tunnel = createConnection({ ...params, decodeStrings: false }, () => {
      this.webserver.emit('log', 'tunnel.connect', {
        traceId: this.traceId,
        streamId: this.id,
        params
      })
    })
    this.once('close', () => this.tunnel.destroy())
    this.tunnel.pipe(this.response)
    this.request.pipe(this.tunnel)
    this.tunnel.once('error', err => {
      this.error(new TunnelError('socket connection error', { error: err }))
    })
    this.tunnel.once('close', transmissionError => {
      this.webserver.emit('log', 'tunnel.close', {
        traceId: this.traceId,
        streamId: this.id,
        transmissionError
      })
  ...
  toJSON () {
    if (this.webserver.config.streamToJSON) {
      return this.webserver.config.streamToJSON(this)
    }
    return {
      id: this.id,
