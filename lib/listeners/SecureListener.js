...
class SecureListener {
  constructor (webserver) {
    ...
    if (webserver.config.key) params.cert = webserver.config.cert
    if (webserver.config.key) params.key = webserver.config.key
    if (webserver.config.SNICallback) params.SNICallback = webserver.config.SNICallback
  ...
  listen (done) {
    this.server.listen(this.webserver.config.securePort, err => {
      ...
      this.webserver.securePort = this.server.address().port
      done()
    })
  }
